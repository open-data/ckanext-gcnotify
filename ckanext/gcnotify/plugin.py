...
class GcnotifyPlugin(plugins.SingletonPlugin):
    ...
    def update_config(self, config):
        ...
        mailer.notify_ckan_user_create = notify_ckan_user_create
...
def get_template_id(action):
    ...
    if (template_ids is None) or not len(template_ids):
      template_ids = {}
    else:
...
def get_request_headers(headers):
    ...
    if (secret_key is None) or not len(secret_key):
...
def get_api_endpoint(endpoint):
    ...
    if (base_uri is None) or not len(base_uri):
